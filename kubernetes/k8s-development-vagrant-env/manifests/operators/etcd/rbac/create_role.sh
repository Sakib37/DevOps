...
#source: https://github.com/coreos/etcd-operator#deploy-etcd-operator
ETCD_OPERATOR_ROOT=$(dirname "$0")
...
sed -e "s/<ROLE_NAME>/${ROLE_NAME}/g" \
  -e "s/<NAMESPACE>/${NAMESPACE}/g" \
  "${ETCD_OPERATOR_ROOT}/templates/cluster-role-template.yaml" > ${ETCD_OPERATOR_ROOT}/cluster-role.yaml
...
sed -e "s/<ROLE_NAME>/${ROLE_NAME}/g" \
  -e "s/<ROLE_BINDING_NAME>/${ROLE_BINDING_NAME}/g" \
  -e "s/<NAMESPACE>/${NAMESPACE}/g" \
  "${ETCD_OPERATOR_ROOT}/templates/cluster-role-binding-template.yaml" > ${ETCD_OPERATOR_ROOT}/cluster-role-binding.yaml
